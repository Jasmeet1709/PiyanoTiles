...
public class MyView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 4; j++) {
                ...
                if (flag) {
                    if (array[4][j] == Color.rgb(63, 81, 181)) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (array[(((int) y + cellHeight) / cellHeight)][(int) x / cellWidth] == Color.rgb(63, 81, 181)) {
            invalidate();
            flag = false;
            callOnDraw = true;
        }
        ...
        if (callOnDraw) {
            if (array[(((int) y + cellHeight - (int) counter1) / cellHeight)][(int) x / cellWidth] == Color.BLACK) {
                array[(((int) y + cellHeight - (int) counter1) / cellHeight)][(int) x / cellWidth] = Color.rgb(96, 96, 96);
